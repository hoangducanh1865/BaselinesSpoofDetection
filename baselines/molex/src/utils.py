...
def _get_optimizer(model_parameters, optim_config):
    ...
    if optimizer_name == 'sgd':
        ...
        optimizer = torch.optim.AdamW(model_parameters,
                                    ...
                                    amsgrad=str_to_bool(
                                        optim_config['amsgrad']))
    elif optimizer_name == 'adamw_separate':
        lora_adapt_param = model_parameters[0]
        class_head_param = model_parameters[1]
        optimizer = torch.optim.Adam(
                                    [
                                    {"params":lora_adapt_param,
                                     "lr":optim_config['lora_lr'],
                                     "weight_decay": optim_config['lora_weight_decay'],
                                     "amsgrad" :str_to_bool(
                                     optim_config['amsgrad'])},

                                    {"params":class_head_param,
                                     "lr":optim_config['base_lr'],
                                     "weight_decay": optim_config['weight_decay'],
                                     "amsgrad" :str_to_bool(
                                     optim_config['amsgrad'])}
                                    ...
                                    betas=optim_config['betas'])                                      
    elif optimizer_name == 'ocsoftmax':
        ocsoftmax = OCSoftmax().to(device)
        ocsoftmax.train()
        # pay attention to the learning rate, is it same as the main rate?
        ocsoftmax_optimzer = torch.optim.SGD(ocsoftmax.parameters(), lr=optim_config['base_lr'])
    else:
