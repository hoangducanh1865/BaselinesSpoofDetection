...
if __name__ == "__main__":
    ...
    labelFile = str(
        Path(
            os.environ.get(
                "MOEF_ASVSPOOF2019_EVAL_PROTOCOL",
                asv2019_root / "ASVspoof2019_LA_cm_protocols" / "ASVspoof2019.LA.cm.eval.trl.txt",
            )
        ).expanduser()
    )
    asvlabel = str(
        Path(
            os.environ.get(
                "MOEF_ASVSPOOF2019_EVAL_ASV_SCORES",
                asv2019_root / "ASVspoof2019_LA_asv_scores" / "ASVspoof2019.LA.asv.eval.gi.trl.scores.txt",
            )
        ).expanduser()
    )
