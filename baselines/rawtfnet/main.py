...
from data_utils_SSL import genSpoof_list, Dataset_ASVspoof2019_train, Dataset_ASVspoof2021_eval, Dataset_ASVspoof2021_eval_no_cut, Dataset_in_the_wild_eval
...
if __name__ == '__main__':
    ...
    # Dataset
    parser.add_argument("--in_the_wild_path", type=str, default='/release_in_the_wild/', help="the path of the folder of in the wild")
    ...
    parser.add_argument('--date', type=str, default='unknown',
                        help='date')
    parser.add_argument('--model_name', type=str, required=True, choices=['rawtfnet','rawtfnet_small'],
    ...
    if args.eval:
        if args.track == 'LA' or track == 'DF':
            ...
            st_t = time.time()
            with torch.no_grad():
                list_IDs = sorted(os.listdir(args.in_the_wild_path))
                eval_set = Dataset_in_the_wild_eval(list_IDs=list_IDs, base_dir=os.path.join(
    ...
    d_label_trn, file_train = genSpoof_list(
        dir_meta=os.path.join(args.protocols_path + 'LA/ASVspoof_LA_cm_protocols/ASVspoof2019.LA.cm.train.trn.txt'),
    ...
    d_label_dev, file_dev = genSpoof_list(
        dir_meta=os.path.join(args.protocols_path + 'LA/ASVspoof_LA_cm_protocols/ASVspoof2019.LA.cm.dev.trl.txt'),
    ...
    dev_set_w_aug = Dataset_ASVspoof2019_train(args, list_IDs=file_dev,
                                         ...
                                         base_dir=os.path.join(
                                             args.database_path + 'LA/ASVspoof2019_LA_dev/'), algo=args.algo)
    dev_set_wo_aug = Dataset_ASVspoof2019_train(args, list_IDs=file_dev,
                                         labels=d_label_dev,
                                         base_dir=os.path.join(
                                             args.database_path + 'LA/ASVspoof2019_LA_dev/'), algo=0)
    dev_loader_w_aug = DataLoader(dev_set_w_aug, batch_size=args.batch_size, num_workers=20, shuffle=False)
    dev_loader_wo_aug = DataLoader(dev_set_wo_aug, batch_size=args.batch_size, num_workers=20, shuffle=False)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        model.eval()
        with torch.no_grad():
            for batch_x, batch_y in tqdm(dev_loader_w_aug):
                batch_size = batch_x.size(0)
                num_total += batch_size
                batch_x = batch_x.to(device)
                batch_y = batch_y.view(-1).type(torch.int64).to(device)
