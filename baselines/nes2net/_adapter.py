...
DEFAULT_WAVLM = Path(
    ...
    "wavlm_large__mrdragonfox__llase_g1/pytorch_model.bin"
)
DEFAULT_XLSR = Path(
    "/home/user14/anhhd/spoof/pretrained_ssl_models/"
...
class Nes2NetEvalDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        return tensor_or_error(
            lambda wav_path: (
                _pad(read_mono_audio(wav_path), self.max_len)
                if self.max_len is not None
                else read_mono_audio(wav_path)
            ),
            utt_id,
            label,
            path,
...
def _run_eval(args, compute_eer: bool) -> None:
    ...
    loader = DataLoader(
        ...
        collate_fn=collate_eval_padded,
    )
