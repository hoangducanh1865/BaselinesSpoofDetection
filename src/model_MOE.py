...
class NoisyTopkRouter(nn.Module):
    ...
    Default behaviour (``routing_type="topk"``) is the original MoLEx noisy
    top-K router. With ``routing_type="entropy"`` it switches to the M2
    "Entropy Routing" ablation (SEE-MoLEx proposal, Section 4.2): an
    input-adaptive number of experts is activated based on the normalized
    Shannon entropy of the router distribution.
    """

    def __init__(self, n_embed, num_experts, top_k, routing_type="topk",
                 tau_max=2.0, tau_min=0.5, k_min=1, k_max=6, warmup_epochs=2):
        super().__init__()
        self.top_k = top_k
        ...
        # Entropy-guided adaptive routing config; a no-op when routing_type != "entropy".
        ...
        # Mutable training state, set per-epoch by the training loop (see main_molex.py).
    ...
    def forward(self, mh_output):
        ...
        if self.routing_type == "entropy":
            return self._entropy_route(noisy_logits)
    ...
    def _entropy_route(self, noisy_logits):
        """Entropy-guided adaptive expert activation (M2).

        Returns (router_output, indices) with the same contract as the top-K
        path, so SparseMoE.forward stays unchanged: experts that are not
        selected simply receive a gating weight of 0.
        """
        num_experts = noisy_logits.size(-1)
        probs = F.softmax(noisy_logits, dim=-1)

        # Warm-up (Eq. 17): activate all routed experts to avoid collapse at init.
        if self.training and self.current_epoch < self.warmup_epochs:
            shape = [1] * (noisy_logits.dim() - 1) + [num_experts]
            indices = torch.arange(num_experts, device=noisy_logits.device).view(*shape).expand(*noisy_logits.shape)
            return probs, indices

        # Normalized Shannon entropy over the routed pool (Eq. 13), in [0, 1].
        entropy = -(probs * torch.log(probs + 1e-8)).sum(dim=-1) / math.log(num_experts)

        # Temperature: linear anneal during training (Eq. 15), tau_min at eval.
        if self.training:
            frac = min(max(float(self.current_epoch) / max(float(self.total_epochs), 1.0), 0.0), 1.0)
            tau = self.tau_max * (1.0 - frac) + self.tau_min * frac
        else:
            tau = self.tau_min
        tau = max(tau, 1e-4)

        # Entropy-derived activation threshold (Eq. 14): gamma in [0.5, 1].
        gamma = 1.0 - 0.5 * torch.pow(entropy, 1.0 / tau)
        max_p = probs.max(dim=-1, keepdim=True).values
        threshold = gamma.unsqueeze(-1) * max_p  # [..., 1]

        # Adaptive activation set (Eq. 16), clamped to [k_min, k_max].
        k_max = min(self.k_max, num_experts)
        k_min = max(min(self.k_min, k_max), 1)
        top_logits, top_idx = noisy_logits.topk(k_max, dim=-1)   # sorted descending
        top_probs = probs.gather(-1, top_idx)

        keep = top_probs >= threshold
        keep[..., :k_min] = True  # always keep the k_min strongest experts

        masked_top_logits = top_logits.masked_fill(~keep, float("-inf"))
        sparse_logits = torch.full_like(noisy_logits, float("-inf"))
        sparse_logits = sparse_logits.scatter(-1, top_idx, masked_top_logits)
        router_output = F.softmax(sparse_logits, dim=-1)
        return router_output, top_idx
...
class Transformer_MOE(TransformerSentenceEncoderLayer):
    ...
    def __init__(
        ...
        self.top_k = topk
        self.ex_hidden_dim = ex_hidden_dim
        self.smoe = SparseMoE(self.embedding_dim, self.num_experts, self.top_k, self.ex_hidden_dim, routing=routing)
...
class Model_MoLEx(nn.Module):
    ...
    def __init__(self, d_args):
        ...
        # Optional routing config for ablations (None -> baseline top-K routing).
        self.routing = d_args.get("routing", None)
